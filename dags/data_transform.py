...
def connect_mongo_postgress():
    client = MongoClient(host= "192.168.0.104", port=27017,username='root',password='rootpassword')
    ...
    conn_string = 'postgresql://airflow:airflow@192.168.0.104/airflow'
...
def connect_minio():
    ...
    ngramdf = pd.DataFrame(dict1)     
    ngramdf.reset_index(level=0, inplace=True)
    del ngramdf["index"]
    for col in dict1.keys():
        ngramdf[col] = ngramdf[col].map(lambda a: ", ".join(a),na_action='ignore')
    ...
    for i in ngramdf.columns:
        ...
        #making the file
        textfile1 = open(process_dir+"\\"+i+"_unigram.txt", "w")
        for element in unigramlist:
            element = str(element)
            textfile1.write(element + "\n")
        textfile1.close()
            
        textfile2 = open(process_dir+"\\"+i+"_bigram.txt", "w")
        for element in bigramlist:
            element = str(element)
            textfile2.write(element + "\n")
        textfile2.close()
        
        textfile3 = open(process_dir+"\\"+i+"_trigram.txt", "w")    
    ...
    found = clientminio.bucket_exists("ngram")
    if not found:
    ...
    for i, filename in enumerate(os.listdir(process_dir)):
        predict_label,lst=tuple(filename.split("_"))
        result = clientminio.fput_object(bucket_name, predict_label+"/"+lst, os.path.join(process_dir,filename))
